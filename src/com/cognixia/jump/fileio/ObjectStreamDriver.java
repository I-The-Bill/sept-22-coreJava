package com.cognixia.jump.fileio;

import java.io.File;
import java.io.FileInputStream;
import java.io.FileOutputStream;
import java.io.InputStream;
import java.io.ObjectInputStream;
import java.io.ObjectOutputStream;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

import com.cognixia.jump.classandobjects.Animal;
import com.cognixia.jump.classandobjects.Horse;

public class ObjectStreamDriver {

	public static void main(String[] args) 
	{
		// TODO Auto-generated method stub
		File file = new File("resources/objFile.data");
		try
		{
			file.createNewFile()
;		}
		catch (Exception e)
		{
			
		}
		
		writeObjToFile(file);
		readObjToFile(file);
	}

	private static void readObjToFile(File file) {
		// TODO Auto-generated method stub
		try (ObjectInputStream reader = new ObjectInputStream(new FileInputStream(file)))
		{
			
			//HAS TO BE DONE IN THE SAME ORDER AS IT WAS WRITTEN IN THE FILE
//			??THIS LIST CORRISPONDS to... 
			Animal animal = (Animal) reader.readObject();
			System.out.println(animal.toString());
			Horse horse = (Horse) reader.readObject();
			System.out.println(horse.toString());
			String hi = (String) reader.readObject();
			System.out.println(hi);
			
			int num = (int) reader.readInt();
			System.out.println(num);
			boolean bool = (boolean) reader.readBoolean();
			System.out.println(bool);
			char ch = (char) reader.readChar();
			System.out.println(ch);
			
			int[] numArray = (int[]) reader.readObject();
			System.out.println(Arrays.toString(numArray));
			
			@SuppressWarnings("unchecked")
			List<String> strList = (List<String>) reader.readObject();
		}
		catch (Exception e)
		{
			e.printStackTrace();
		}
	}

	private static void writeObjToFile(File file) {
		// TODO Auto-generated method stub
		Animal animal = new Animal("Whale", 2500.0);
		Horse horse = new Horse();
		String hello = "Hello World";
		
		try(ObjectOutputStream writer = new ObjectOutputStream(new FileOutputStream(file)))
		{
			writer.writeObject(animal);
			writer.writeObject(horse);
			writer.writeObject(hello);
			int num = 4;
			boolean bool = true;
			char ch = '#';
			
			writer.writeInt(num);
			writer.writeBoolean(bool);
			writer.writeChar(ch);
			
			int[] numArr = {1,3,5,7};
			writer.writeObject(numArr);
			
			List<String> strList = new ArrayList<String>();
			strList.add("str1");
			strList.add("Str2");
			writer.writeObject(strList);
		}
		catch(Exception e)
		{
			e.printStackTrace();
		}
	}

}
